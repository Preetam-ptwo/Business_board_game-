...
public class board_controller implements Initializable{
	...
	String uname = "root";
	String password = "i am ptwo";
	
	
	public void roll_dice(ActionEvent event) {
		if(bought==1) 
		{update_turn();bought=0;}
		...
		//@FXML
		Image dice_img1 = new Image(getClass().getResourceAsStream("/application/images/dice"+d1+".png"));	//update dice images
	...
	public void buy_land() {
		try {
			Connection con = DriverManager.getConnection(url,uname,password);
				...
				ResultSet r = s1.executeQuery("SELECT buy FROM land where lid="+(player_pos[turn]+1));//getting land's buying amt from db
				r.next();
				int l_amt = r.getInt(1);
				ResultSet r1 = s1.executeQuery("SELECT amount FROM players WHERE pid="+turn);//getting players balance amount from db
				r1.next();
				...
				if(l_amt<=p_amt)
				{
					...
					ResultSet am = s1.executeQuery("SELECT amount FROM players WHERE pid="+turn);//select current balance amount to display
					am.next();
					int amt = am.getInt(1);
					switch(turn) {									//displaying players's amount after deducting
	...
	public void update_owners(int p,Label a) {
		try {
			...
			if(!b) {	
				if(rent==0 || rent==Types.NULL){	//if they land in either chance or community chest or free parking
				...
				else {							//no owners of the land so ask the player if he wants to buy the land
				...
				if(own_pid!=turn) {
					...
					int amt = am.getInt(1);
					a.setText(String.valueOf(amt));
					am = s.executeQuery("SELECT amount FROM players WHERE pid="+own_pid);am.next();
					int amtr = am.getInt(1);
					switch(own_pid) {
					case 1:amt1.setText(String.valueOf(amtr));break;
					case 2:amt2.setText(String.valueOf(amtr));break;
					case 3:amt3.setText(String.valueOf(amtr));break;
					case 4:amt4.setText(String.valueOf(amtr));break;
					}
